    ...
    <v-tabs-items v-model="coursesTab">
      <v-tab-item>
        ...
        <div v-if="addCourseField" class="mb-2 pa-5">
          ...
          <div class="d-flex">
            <v-select class="w-100" label="Seleziona Corso" v-model="addCourse.id" outlined dense hide-details :items="coursesList" item-text="name" item-value="id"></v-select>
            <v-menu
...
export default {
  ...
  methods: {
    ...
    async createCourseField () {
      ...
      this.coursesList = (await Axios.get('http://localhost:8000/api/courses')).data.data
